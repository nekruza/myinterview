...
function GoogleAuth() {
  ...
  async function handleGoogleSignIn() {
    setLoading(true);
    setError(null);
    track("Auth Started", { method: "google", plan: plan ?? "free" });
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-sm" style={{ color: "rgba(255,255,255,0.42)" }}>
          {plan
            ? `Continue with Google to start your ${plan} plan — 3 free sessions included.`
      ...
      <p className="text-center text-xs mt-6" style={{ color: "rgba(255,255,255,0.32)" }}>
        Same button works for new and existing users — no separate sign-in step.
      </p>
    </div>
  );
}
